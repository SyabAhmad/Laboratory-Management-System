...
<div class="modal fade" id="patientIdModal" tabindex="-1" role="dialog" aria-labelledby="patientIdModalLabel" aria-hidden="true">
            ...
            <div class="modal-body text-center py-4">
                ...
                <div class="alert alert-warning mt-4">
                    <small>
                        ...
                        <strong>Important:</strong> Enter this Patient ID on the CBC analyzer (PID-3 field) before running the test.
                    </small>
                </div>
            </div>
            <div class="modal-footer">
...
$(function () {
            ...
            if (result.value === true) {
                $.ajax({
                    ...
                    success: function(data) {
                        Swal.fire({title: 'Deleted!', text: 'Patient record has been deleted.', icon: 'success', showConfirmButton: false, timer: 1500});
                        table.draw();
                    },
                    error: function(data) {
                        Swal.fire({title: 'Error!', text: 'Something went wrong', icon: 'error', showConfirmButton: false, timer: 1500});
                    }
                });
            }
        });
    ...
    $('body').on('click', '.editbtn', function() {
        ...
        $.get("{{ url('patients') }}/" + id + "/edit").done(function(res) {
            ...
            Swal.fire({title: 'Error!', text: 'Failed to load patient data', icon: 'error', showConfirmButton: false, timer: 1500});
        });
    });
    ...
    $('#editPatientForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                Swal.fire({title: 'Success!', text: 'Patient updated successfully', icon: 'success', showConfirmButton: false, timer: 1500});
                table.draw();
            },
            error: function(xhr) {
                Swal.fire({title: 'Error!', text: 'Failed to update patient', icon: 'error', showConfirmButton: false, timer: 1500});
            }
        });
    });
...
window.copyPatientId = function () {
    ...
    if (typeof Swal !== 'undefined') {
        Swal.fire({ icon: 'success', title: 'Copied!', text: 'Patient ID copied to clipboard', timer: 1500, showConfirmButton: false });
    }
};

window.printPatientId = function () {
    const printContent = document.getElementById('printLabelTemplate').innerHTML;
    const html = `<!DOCTYPE html><html><head><title>Patient Label</title><style>body{font-family:Arial,sans-serif;margin:20px}@media print{body{margin:0}}</style></head><body>${printContent}</body></html>`;
